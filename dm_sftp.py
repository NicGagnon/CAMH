    ...
    for iloc in range(len(mrusers)):
        ...
        with pysftp.Connection(mrserver,
            ...
            for valid_dir in valid_dirs:
                ...
                process_dir(sftp, valid_dir, zips_path, cfg)
...
def process_dir(connection, directory, zips_path, cfg):
    ...
    with connection.cd(directory):
        ...
        for file_name in files:
            ...
            if check_exists_isnewer(connection, file_name, target, cfg):
...
def check_exists_isnewer(sftp, filename, target, cfg):
    """Cross checks the upload_check.csv to check if the file has already been uploaded to XNAT. If so, then returns False"""
    upload_path = cfg.get_study_base() + "/metadata/upload_check.csv"
    with open(upload_path, 'rt') as f:
        reader = csv.reader(f, delimiter=',')
        for row in reader:
            if filename == row[0] or filename[:-5] == row [0]: # if the username shall be on column 3 (-> index 2)
                print "{} has already been uploaded to XNAT".format(str(filename))
                return False
